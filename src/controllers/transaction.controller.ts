...
export class TransactionController {
  ...
  // Methods for handling transactions will be added here in the future
    ...
    @Get("/")
    async findManyByFilter(@Query() query:transactionFilterBody){
            ...
            return {
                ...
                meta:{
                    ...
                    links:{
                        ...
                        last:`${this.baseUrl}?page=${Math.ceil(data.length / pageSize)}&pageSize=${pageSize}`,
                        next: page < Math.ceil(data.length / pageSize) ? `${this.baseUrl}?page=${Number(page) + 1}&pageSize=${pageSize}` : null,
                ...
                links:{
                    first:`${this.baseUrl}?page=1&pageSize=${pageSize}`,
                    last:`${this.baseUrl}?page=${Math.ceil(data.length / pageSize)}&pageSize=${pageSize}`,
                    next: page < Math.ceil(data.length / pageSize) ? `${this.baseUrl}?page=${page + 1}&pageSize=${pageSize}` : null,
